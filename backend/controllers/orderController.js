  ...
  try {
    ...
    line_items.push({
      price_data: {
        ...
        unit_amount: 2 * 100 // Rs.2 delivery charge in paise
      },
      quantity: 1
    });
    ...
    const session = await stripe.checkout.sessions.create({
      line_items, // correct spelling: sessions.create and line_items
      mode: 'payment',
      success_url: `${frontend_url}/verify?success=true&orderId=${newOrder._id}`,
      cancel_url: `${frontend_url}/verify?success=false&orderId=${newOrder._id}`
    });
...
const updateStatus = async (req,res) =>{
  ...
  try {
    await orderModel.findByIdAndUpdate(req.orderId,{status:req.status});
